...
;;; The pc (program counter) register determines the sequencing of
...
;;; Consider alternative implementation in p. 632, footnote 4
...
(define (lookup-label labels label-name)
  (let ((val (assoc label-name labels)))
    (if val
        (cdr val)
        (error "Undefined label: lookup-label assemble"
               label-name))))

;;; p. 634 Generating execution procedures for instructions
(define (make-execution-procedure
  ...
  (cond ((eq? (car inst) 'assign)
        ...
        (else
         (error "Unknown instruction type: make-execution-procedure ASSEMBLE" inst))))
    ...
    (if (operation-exp? condition)
        (let ((condition-proc
          ...
          (lambda ()
            ...
            (advance-pc pc)))
        (error "Bad MAKE-TEST instruction: ASSEMBLE" inst))))
        ...
        (let ((insts
          ...
          (lambda ()
            (if (get-contents flag)
                ...
                (advance-pc pc))))
        (error "Bad MAKE-BRANCH instruction: ASSEMBLE" inst))))
...
(define (make-goto inst machine labels pc)
  (let ((dest (goto-dest inst)))
    (cond ((label-exp? dest)
          ...
          (else (error "Bad MAKE-GOTO instruction: ASSEMBLE"
                       inst)))))
(define (goto-dest goto-instruction)
  (cadr goto-instruction))

;;; p. 638 Other instructions
...
(define (make-perform inst machine labels operations pc)
  (let ((action (perform-action inst)))
    (if (operation-exp? action)
        ...
        (error "Bad MAKE-PERFORM instruction: ASSEMBLE" inst))))
(define (perform-action inst) (cdr inst))

;;; p. 639 Execution procedures for subexpressions
